package com.marketSpring.board.freeBo.vo;

import java.sql.Date;

public class FreeBoardImgVO {
	private int free_bo_img_no;
	private int free_bo_no;
	private String img_filename;
	private Date reg_date;
	private int is_del;
	
	public int getFree_bo_img_no() {
		return free_bo_img_no;
	}
	public void setFree_bo_img_no(int free_bo_img_no) {
		this.free_bo_img_no = free_bo_img_no;
	}
	public int getFree_bo_no() {
		return free_bo_no;
	}
	public void setFree_bo_no(int free_bo_no) {
		this.free_bo_no = free_bo_no;
	}
	public String getImg_filename() {
		return img_filename;
	}
	public void setImg_filename(String img_filename) {
		this.img_filename = img_filename;
	}
	public Date getReg_date() {
		return reg_date;
	}
	public void setReg_date(Date reg_date) {
		this.reg_date = reg_date;
	}
	public int getIs_del() {
		return is_del;
	}
	public void setIs_del(int is_del) {
		this.is_del = is_del;
	}
	
}
